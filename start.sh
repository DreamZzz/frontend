...
start_metro_background() {
    ...
    npx react-native start --reset-cache > /tmp/social-app-metro.log 2>&1 &
    METRO_PID=$!
    ...
    if [ "$(uname)" = "Darwin" ]; then
        ...
        if [ ! -d "ios/Pods" ]; then
            ...
            if ! $BUNDLE_CMD exec pod install --no-repo-update; then
                ...
                print_info "   cd /Users/qiang/social-app-ios/frontend"
...
check_ios_device_signing_ready() {
    ...
    if [ "$identity_count" = "0" ]; then
        ...
        print_info "5. 保持 Bundle Identifier 为唯一值（当前已设为 com.qiang.socialapp.frontend）"
